...
function registerAdmin()
{
    ...
    $alamat = htmlspecialchars($_POST['alamat']);
    $tgl_lahir = $_POST['tgl_lahir'];
    $query_user = "INSERT INTO adminsimak VALUES ('', '0', '$username','$nama', '$email', '$alamat', '$tgl_lahir','$password')";
    $exe = mysqli_query($koneksi, $query_user);
...
function login() {
    ...
    $query3 = mysqli_query($koneksi, "SELECT * FROM adminsimak where username = '$username'");
...
function editAdmin($id_admin)
{
    ...
    $username = htmlspecialchars($_POST['username']);
    $email = htmlspecialchars($_POST['email']);
    $queryProfil = "UPDATE adminsimak SET nama = '$nama', username = '$username', email = '$email' WHERE id_admin = '$id_admin'";
    $exe = mysqli_query($koneksi, $queryProfil);
        if(!$exe){
            die('Error pada database');
        }    
